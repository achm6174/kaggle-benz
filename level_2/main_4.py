...
# ======================== NN optimize ======================== #
def f(coord,args):
    pred_1, pred_2, pred_3, pred_4 = args
    return -r2_score(df_train.y.values, coord[0]*(df_1.y.values) + coord[1]*(df_2.y.values) + coord[2]*(df_3.y.values) + coord[3]*(df_4.y.values))


initial_guess = np.array([1/4. for x in range(4)])
...
tmp = res.x[0]*(df_1.y.values)  + res.x[1]*(df_2.y.values) + res.x[2]*(df_3.y.values)  + res.x[3]*(df_4.y.values)
...
tmp_test = (res.x[0]*(df_1_test.y.values)  + res.x[1]*(df_2_test.y.values) + res.x[2]*(df_4_test.y.values))

df_test = df_1_test.copy()
df_test['y'] = tmp_test

df_test.to_csv("level_2/cache/test/main_3.csv", index=False)
